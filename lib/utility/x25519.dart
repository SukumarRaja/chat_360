...
class X25519 {
  static final Uint8List constant9 = () {
    final result = Uint8List(32);
    result[0] = 9;
    return result;
  }();

  static final Int32List constant121665 = () {
  ...
  Key calculateSharedSecretSync(Key secretKey, Key publicKey) {
    ...
    final result = Uint8List(32);
    scalarMultiply(result, secretKeyTransformed, publicKey.bytes!);
    return Key(result);
  }
  ...
  AsymmetricKeyPair generateKeyPairSync({Uint8List? seed}) {
    ...
    scalarMultiply(publicKeyBytes, seed, constant9);
  ...
  static void conditionalSwap(Int32List p, Int32List q, int b) {
    final c = ~(b - 1);
    for (var i = 0; i < 16; i++) {
      final t = c & (p[i] ^ q[i]);
      p[i] ^= t;
  ...
  static void multiply(Int32List result, Int32List a, Int32List b) {
    var t0 = 0,
        t1 = 0,
        t2 = 0,
        t3 = 0,
